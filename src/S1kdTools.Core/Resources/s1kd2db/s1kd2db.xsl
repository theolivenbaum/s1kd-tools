<?xml version="1.0"?>
<!-- PORT NOTE (s1kd-tools C# port): the single call to the XSLT 1.0
     function unparsed-entity-uri() has been replaced by ier:resolve(), bound at
     runtime to a .NET extension object (S1kdTools.DocBook.EntityUriResolver,
     registered under the 'InfoEntityResolver' namespace). System.Xml's
     XslCompiledTransform does not implement unparsed-entity-uri(); the shim
     resolves the graphic entity name to its NDATA system id from the source
     document's DTD (and, for s1000dtodb, the optional info-entity-map), which
     matches the original behaviour. This is the only modification to the
     upstream stylesheet. -->
<xsl:stylesheet
  xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  xmlns:d="http://docbook.org/ns/docbook"
  xmlns:ier="InfoEntityResolver"
  xmlns:mml="http://www.w3.org/1998/Math/MathML"
  xmlns:xlink="http://www.w3.org/1999/xlink"
  exclude-result-prefixes="mml xlink ier"
  version="1.0">
  ...
  <xsl:template name="image.object">
    <d:imageobject>
      <d:imagedata fileref="{ier:resolve(string(@infoEntityIdent))}">
