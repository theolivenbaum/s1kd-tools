...
public sealed class AsppTool : ITool
{
    ...
    /// <summary>The text of the custom display-text XSLT supplied with -x, or null.</summary>
    private string? _customXslText;
    ...
    public int Run(IReadOnlyList<string> args, TextWriter stdout, TextWriter stderr)
    {
        ...
        // When -x is given, display text is generated by applying a user-supplied
        // XSLT to a <mux> of the DM + ACTs + CCTs (mirrors the custom
        // styledoc/generateDisplayText path in the C). Otherwise the built-in
        // DOM/.disptext path is used.
        if (customGenDispTextFile != null)
        {
            try
            {
                _customXslText = File.ReadAllText(customGenDispTextFile);
            }
            catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
            {
                stderr.WriteLine($"{Name}: ERROR: Could not read XSLT: {customGenDispTextFile}: {ex.Message}");
                return 2;
            }
        }

        // Load the .disptext configuration: explicit -G, else a discovered
        // .disptext in the directory tree, else the built-in default. Not needed
        // when a custom XSLT drives display-text generation.
        DispTextConfig? dispCfg = null;
        if (_customXslText == null)
        {
            ...
            dispCfg = new DispTextConfig(disptext, format);
        }
        ...
        TextWriter stdout, TextWriter stderr)
    {
        ...
        else if (genDispText)
        {
            if (_customXslText != null)
            {
                GenerateDisplayTextWithXsl(doc, allActs, allCcts, _customXslText, stderr);
            }
            else
            {
    ...
    /// <summary>
    /// Generate display text by applying a user-supplied XSLT (the <c>-x</c>
    /// option). Mirrors the C <c>generateDisplayText</c>: a <c>&lt;mux&gt;</c>
    /// document is built containing the data module plus all referenced ACTs and
    /// CCTs, the stylesheet is applied with the <c>overwrite-display-text</c>
    /// parameter, and the transformed data module (<c>/mux/*</c>) replaces the
    /// original root.
    /// </summary>
    private void GenerateDisplayTextWithXsl(XmlDocument doc, List<string> acts, List<string> ccts, string xslText, TextWriter stderr)
    {
        if (doc.DocumentElement == null)
        {
            return;
        ...
        // Build <mux><dmodule/><acts>...</acts><ccts>...</ccts></mux>.
        var mux = new XmlDocument { PreserveWhitespace = true };
        XmlElement muxRoot = mux.CreateElement("mux");
        mux.AppendChild(muxRoot);

        muxRoot.AppendChild(mux.ImportNode(doc.DocumentElement, true));

        XmlElement muxActs = mux.CreateElement("acts");
        muxRoot.AppendChild(muxActs);
        foreach (XmlDocument act in LoadDocs(acts, stderr))
        {
            if (act.DocumentElement != null)
            {
                muxActs.AppendChild(mux.ImportNode(act.DocumentElement, true));
            }
        }

        XmlElement muxCcts = mux.CreateElement("ccts");
        muxRoot.AppendChild(muxCcts);
        foreach (XmlDocument cct in LoadDocs(ccts, stderr))
        {
            if (cct.DocumentElement != null)
            {
                muxCcts.AppendChild(mux.ImportNode(cct.DocumentElement, true));
            }
        }
        ...
        try
        {
            ...
            xslt.Load(styleReader, new XsltSettings(enableDocumentFunction: false, enableScript: false), null);
        }
        catch (XsltException ex)
        {
            if (_verbosity >= 1)
            {
                stderr.WriteLine($"{Name}: ERROR: Invalid XSLT: {ex.Message}");
            }
            return;
        }

        var args = new XsltArgumentList();
        args.AddParam("overwrite-display-text", string.Empty, _overwriteDispText);
        ...
        using (var ms = new MemoryStream())
        {
            ...
            {
                xslt.Transform(mux, args, writer);
            }
            ms.Position = 0;
            var readerSettings = new XmlReaderSettings
            {
        ...
        // The transformed data module is /mux/*[1].
        ...
        XmlNode imported = doc.ImportNode(newRoot, true);
