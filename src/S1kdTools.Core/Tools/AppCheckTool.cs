...
/// elements), detects broken <c>internalRef</c> cross references to filtered-out
/// content, and then runs the validators in-process: the default schema
/// validator (<see cref="ValidateTool"/>) always, the BREX check
/// (<see cref="BrexCheckTool"/>) when <c>-b</c> is set, and custom validators
/// (<c>-e</c>) dispatched through <see cref="ToolRegistry"/>. CCT dependency
/// injection (<c>-~</c>), parallel threads (<c>-#</c>), the
...
public sealed class AppCheckTool : ITool
{
    ...
    public int Run(IReadOnlyList<string> args, TextWriter stdout, TextWriter stderr)
    {
        ...
        // CCT-dependency injection (-~) and omit-issue (-N) filename handling are
        // parsed for compatibility but not fully ported; warn when requested so
        // behaviour is not silently different. The default schema validator, the
        // BREX check (-b) and any custom validators (-e) ARE ported in-process
        // (see RunValidators).
        if ((_addDeps || _noIssue) && _verbosity >= Verbosity.Verbose)
        {
            stderr.WriteLine($"{MsgPrefix}: WARNING: Options -~/-N are not fully supported in this port.");
        }
