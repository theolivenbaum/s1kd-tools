...
/// object. The non-chapterized IPD SNS construction (-b) is also ported: missing
/// SNS components on a CSN/IPD reference are filled in from the supplied SNS (or
/// inherited from the containing DM when the SNS is "-"), so non-chapterized CSN
/// refs resolve to an IPD DMC. ICN entity rewriting on update (-U of an
/// infoEntityIdent) is not performed because System.Xml's entity handling
/// differs from libxml2.
/// </summary>
