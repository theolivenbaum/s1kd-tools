      ...
      <fo:table table-layout="fixed" width="{$body-w}mm">
        ...
        <fo:table-body>
          <fo:table-row>
            <fo:table-cell>
              <!--
                The publisher is a wordmark when it is short and a line of text
                when it is a full department name; sizing it by length keeps a
                long name from pushing the header out of its region.
              -->
      ...
      <xsl:when test="@s1kdResolvedGraphic">
        <fo:block>
          <fo:external-graphic src="url('{@s1kdResolvedGraphic}')"
                               ...
                               width="{$body-w - 10}mm" height="150mm"
                               scaling="uniform"/>
        </fo:block>
      </xsl:when>
      <xsl:otherwise>
